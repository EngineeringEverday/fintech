...
> **Note:** `data/settleiq.db` is excluded from version control (see `.gitignore`). Regenerate it in under 1 second with `python data/mock_generator.py`. `data/merchant_registry.json` is also regenerated by the same command.
